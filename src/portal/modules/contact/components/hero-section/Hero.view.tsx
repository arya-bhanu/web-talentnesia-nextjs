...
export const HeroSectionView = ({ className, isLoading }: { className?: string, isLoading?: boolean }) => {
  return (
    <section
      className={
        clsx(className) +
        'bg-secondary min-h-[65vh] md:min-h-[60vh] lg:min-h-[65vh] space-y-10'
      }
    >
      <div className="container pt-10 md:pt-14 lg:pt-24 flex flex-col items-center lg:items-start">
        <div className="text-center lg:text-left pt-10 md:pt-15">
          <SkeletonLoader visible={isLoading ? isLoading : false} width={'20%'} height={25}/>
          {
            !isLoading &&
            <>
          ...
          <div className="w-full lg:w-[60%] space-y-10 md:space-y-15 lg:pt-8" style={isLoading === true ? {minWidth: 450} : undefined}>
            <SkeletonLoader visible={isLoading ? isLoading : false} width={'60%'} height={42} containerStyle={{marginTop: 72}} />
            <SkeletonLoader visible={isLoading ? isLoading : false} />
            <SkeletonLoader visible={isLoading ? isLoading : false} width={'70%'} containerStyle={{marginTop: 10}}/>
            {
              !isLoading && 
              <>
        ...
        <div className="w-full grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-2 pt-10 lg:pt-14 mb-10 xl:mb-0">
          {heroSectionData.map((item, index) => (
            <div
              key={index}
              className="flex flex-row items-start lg:items-start text-left space-x-2"
            >
              <SkeletonLoader visible={isLoading ? isLoading : false} variant='circle-image' height={50} width={50} containerStyle={{marginRight: 16, minWidth: 50}}/>
              {
                !isLoading && 
                <>
              ...
              <div style={isLoading === true ? {minWidth: 300} : undefined}>
                <SkeletonLoader visible={isLoading ? isLoading : false}  height={18} />
                <SkeletonLoader visible={isLoading ? isLoading : false} width={'35%'} height={10} containerStyle={{marginTop: 15}} />
                {
                  !isLoading &&
                  <>
