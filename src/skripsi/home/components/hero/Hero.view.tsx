...
const HeroSectionView = ({ className, isLoading }: { className?: string, isLoading?: boolean }) => {
    ...
    >
      <div className="flex container pt-28 md:pt-32 lg:pt-36 xl:pt-40">
        <div className="flex-[1]">
          ...
          <SkeletonLoader visible={isLoading ? isLoading : false} width={'65%'} height={40}/>
          <SkeletonLoader visible={isLoading ? isLoading : false} width={'90%'} height={40}/>
          <SkeletonLoader visible={isLoading ? isLoading : false} height={10} containerStyle={{marginTop: 50}}/>
          <SkeletonLoader visible={isLoading ? isLoading : false} height={10} width={'105%'}/>
          <SkeletonLoader visible={isLoading ? isLoading : false} height={10} width={'50%'}/>
          <SkeletonLoader visible={isLoading ? isLoading : false} width={'55%'} height={30} containerStyle={{marginTop: 56}}/>
          {
            !isLoading &&
            <>
        ...
        <div className="flex-[2] justify-end lg:flex hidden">
          <SkeletonLoader visible={isLoading ? isLoading : false} width={450} height={450} variant='circle-image' />
          {
            !isLoading &&
            <Image
              alt="hero image"
      ...
      <SkeletonLoader visible={isLoading ? isLoading : false} width={'30%'} containerStyle={{
        marginLeft: 275, marginTop: 0
      }}/>
      {
        !isLoading && 
