...
const minioAccessSchema = z.object({
    MINIO_ACCESS_KEY_ID: z
        ...
        .refine((value) => value.trim().length > 0, { message: 'MINIO_ACCESS_KEY_ID cannot contain only whitespace' }),
    MINIO_SECRET_ACCESS_KEY: z
        .string()
        .min(1, { message: 'MINIO_SECRET_ACCESS_KEY cannot be empty' })
        .refine((value) => value.trim().length > 0, { message: 'MINIO_SECRET_ACCESS_KEY cannot contain only whitespace' }),
    MINIO_BUCKET: z
        .string()
        .min(1, { message: 'MINIO_BUCKET cannot be empty' })
        .refine((value) => value.trim().length > 0, { message: 'MINIO_BUCKET cannot contain only whitespace' }),
    MINIO_DEFAULT_REGION: z
        .string()
        .min(1, { message: 'MINIO_DEFAULT_REGION cannot be empty' })
        .refine((value) => value.trim().length > 0, { message: 'MINIO_DEFAULT_REGION cannot contain only whitespace' }),
    MINIO_ENDPOINT: z
        .string()
        .min(1, { message: 'MINIO_ENDPOINT cannot be empty' })
        .url({ message: 'MINIO_ENDPOINT must be a valid URL' })
        .refine((value) => value.trim().length > 0, { message: 'MINIO_ENDPOINT cannot contain only whitespace' }),
    MINIO_URL: z
        .string()
        .min(1, { message: 'MINIO_URL cannot be empty' })
        .url({ message: 'MINIO_URL must be a valid URL' })
        .refine((value) => value.trim().length > 0, { message: 'MINIO_URL cannot contain only whitespace' })
})
...
const uploadMinio = async (file: File, group: string, directory: string): Promise<string> => {
    try {
