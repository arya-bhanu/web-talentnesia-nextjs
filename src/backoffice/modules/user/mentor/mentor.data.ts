// Static dropdown options data
export const religions = [
  'Islam',
  'Kristen',
  'Katholik',
  'Hindu',
  'Buddha',
  'Lainnya',
];

export const isMarried = ['Belum Menikah', 'Menikah', 'Bercerai', 'Janda'];

export const titles = ['Sarjana (S1)', 'Magister (S2)', 'Doktor (S3)'];

export const provinces = ['Jawa Timur', 'jawa Barat', 'Jawa Tengah'];

export const districts = ['Malang', 'Cirebon', 'Semarang'];

export const subDistrict = ['KedungKandang', 'Depok', 'Gunungpati'];

export const placesOfBirth = [
  'Jakarta',
  'Surabaya',
  'Bandung',
  'Medan',
  'Semarang',
  'Makassar',
  'Palembang',
  'Yogyakarta',
];
