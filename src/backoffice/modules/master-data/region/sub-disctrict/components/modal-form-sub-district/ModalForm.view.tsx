      ...
      <form onSubmit={(e) => { e.preventDefault(); handleSave(); }}>
        <Modal.Body>
          <div className="space-y-4">
            <div>
              <label className="block mb-2 text-sm font-medium text-gray-900">Sub district name<label className="text-red-500">*</label></label>
              <input
                type="text"
                placeholder="Input sub district name"
                value={formData.subDistrict || ''}
                onChange={(e) => handleInputChange('subDistrict', e.target.value)}
                required
                className={`block w-full p-2 border ${hasError && !formData.subDistrict ? 'border-red-500' : 'border-gray-300'} rounded-lg`}
              />
              {hasError && !formData.subDistrict && (
                <p className="text-red-500 text-xs mt-1">Sub district name is required.</p>
              )}
            </div>
          </div>
        </Modal.Body>
