      ...
      <form onSubmit={(e) => { e.preventDefault(); handleSave(); }}>
        <Modal.Body>
          <div className="space-y-4">
            <div>
              <label className="block mb-2 text-sm font-medium text-gray-900">Province name<label className="text-red-500">*</label></label>
              <input
                type="text"
                ...
                required
                className={`block w-full p-2 border ${hasError && !formData.province ? 'border-red-500' : 'border-gray-300'} rounded-lg`}
              />
              {hasError && !formData.province && (
                <p className="text-red-500 text-xs mt-1">Province name is required.</p>
              )}
            </div>
          </div>
        </Modal.Body>
