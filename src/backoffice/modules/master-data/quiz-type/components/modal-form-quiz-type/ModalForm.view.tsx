      ...
      <form onSubmit={(e) => { e.preventDefault(); handleSave(); }}>
        <Modal.Body>
          <div className="space-y-4">
            <div>
              <label className="block mb-2 text-sm font-medium text-gray-900">Quiz type name<label className="text-red-500">*</label></label>
              <input
                type="text"
                placeholder="Input quiz type name"
                value={formData.quizType || ''}
                onChange={(e) => handleInputChange('quizType', e.target.value)}
                required
                className={`block w-full p-2 border ${hasError && !formData.quizType ? 'border-red-500' : 'border-gray-300'} rounded-lg`}
              />
              {hasError && !formData.quizType && (
                <p className="text-red-500 text-xs mt-1">Quiz type name is required.</p>
              )}
            </div>
          </div>
        </Modal.Body>
